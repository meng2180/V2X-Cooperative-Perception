...
def visualize(infer_result, pcd, pc_range, save_path, method='3d', left_hand=False):
    ...
    attrib_list = ['pred_box_tensor', 'pred_score', 'gt_box_tensor', 'score_tensor', 'ego_pred_box_tensor']
    ...
    ego_pred_box_tensor = infer_result.get("ego_pred_box_tensor", None)
    ...
    if ego_pred_box_tensor is not None:
        ego_pred_box_np = ego_pred_box_tensor.cpu().numpy()
        ego_pred_name = ['ego_pred'] * ego_pred_box_np.shape[0]

        ego_score = infer_result.get("ego_score_tensor", None)
        if ego_score is not None:
            ego_score_np = ego_score.cpu().numpy()
            ego_pred_name = [f'ego_score:{ego_score_np[i]:.3f}' for i in range(ego_score_np.shape[0])]

    if gt_box_tensor is not None:
        gt_box_np = gt_box_tensor.cpu().numpy()
        gt_name = ['gt'] * gt_box_np.shape[0]

    mode_list = ['gt', 'cp', 'ego_pred', 'gt+cp+ego_pred']
    canvas_list = []
    ...
    if method == 'bev':
        for mode in mode_list:
            ...
            if gt_box_tensor is not None and ('gt' in mode):
                canvas.draw_boxes(gt_box_np, colors=(0, 255, 0))
            if pred_box_tensor is not None and ('cp' in mode):
                canvas.draw_boxes(pred_box_np, colors=(255, 0, 0))
            if ego_pred_box_tensor is not None and ('ego_pred' in mode):
                canvas.draw_boxes(ego_pred_box_np, colors=(0, 0, 255))
        ...
        for mode in mode_list:
            ...
            if pred_box_tensor is not None and ('cp' in mode):
                canvas.draw_boxes(pred_box_np, colors=(255, 0, 0))
            if ego_pred_box_tensor is not None and ('ego_pred' in mode):
                canvas.draw_boxes(ego_pred_box_np, colors=(0, 0, 255), texts=ego_pred_name)
        ...
        canvas = canvas_3d.Canvas_3D(camera_center_coords=(0, 0, 2.3), camera_focus_coords=(0 + 0.9396926, 0, 2.3 + 0.9396926), 
                                    left_hand=left_hand, canvas_shape=(500, 1500))
        canvas_xy, valid_mask = canvas.get_canvas_coords(pcd_np)
        canvas.draw_canvas_points(canvas_xy[valid_mask])
        if gt_box_tensor is not None:
            canvas.draw_boxes(gt_box_np, colors=(0, 255, 0))
        ...
        raise(f"Not Completed for {method} visualization.")
    ...
    ax2.set_title('CP', fontsize=15)
    ax2.axis('off')

    ax3 = plt.subplot(223)
    ax3.imshow(canvas_list[2].canvas)
    ax3.set_title('Ego Preds', fontsize=15)
    ax3.axis('off')

    ax4 = plt.subplot(224)
    ax4.imshow(canvas_list[3].canvas)
    ax4.set_title('GT with CP and Ego Pred', fontsize=15)
    ax4.axis('off')
