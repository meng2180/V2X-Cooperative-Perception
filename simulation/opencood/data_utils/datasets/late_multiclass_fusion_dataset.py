...
from opencood.utils.pose_utils import add_noise_data_dict, add_noise_data_dict_asymmetric_v2
...
'''
通过 __getitem__ 和 __getitem_single_class__ 获取单帧数据。
通过 get_item_single_car 处理单个 CAV 的数据。
通过 collate_batch_train 和 collate_batch_test 将数据整理成适合训练和测试的格式。
通过 post_process 等方法将模型输出转换为边界框。
'''
...
def getLatemulticlassFusionDataset(cls):
    ...
    class LatemulticlassFusionDataset(cls):
        ...
        def __getitem__(self, idx, extra_source=None, data_dir=None):
            ...
            if True: # self.multiclass and extra_source is None:
        ...
        def __getitem_single_class__(self, idx, tpe=None, extra_source=None, data_dir=None):
            ...
            base_data_dict = add_noise_data_dict_asymmetric_v2(base_data_dict,self.params['noise_setting'])
            ...
            for cav_id in cav_id_list:
                ...
                if True: # self.supervise_single  and extra_source is None :
        ...
        # 使用gt替代pred
        def post_process_multiclass_gt(self, data_dict, output_dict, online_eval_only=False):
            """
            Process the outputs of the model to 2D/3D bounding box.
            ...
            for i in range(num_class):
                ...
                pred_box_tensor = self.post_processor.generate_gt_bbx(gt_dict_single)
                gt_box_tensor = self.post_processor.generate_gt_bbx(gt_dict_single)

                pred_box_tensor_list.append(pred_box_tensor)
                gt_box_tensor_list.append(gt_box_tensor)
            pred_score_list = torch.ones(len(gt_box_tensor_list))
            return pred_box_tensor_list, pred_score_list, gt_box_tensor_list
