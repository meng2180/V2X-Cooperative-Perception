...
def inference_single_from_intermediate_fusion_multiclass(batch_data, model, dataset, online_eval_only=False):
    ...
    for cav_id, cav_content in batch_data.items():
        # model = checkpoint.***.perception.config_yaml.model.core_method (e.g: center_point_codriving)
        output_dict[cav_id] = model(cav_content)
...
def inference_no_fusion_multiclass_v2xvit_fcooper(batch_data, model, dataset, online_eval_only=False):
    
    output_dict_ego = OrderedDict()
    cav_content = {'ego': copy.deepcopy(batch_data['ego'])}
    model.no_fusion = True
    output_dict_ego['ego'] = model(cav_content['ego'])
    model.no_fusion = False

    pred_box_tensor, pred_score, gt_box_tensor = \
        dataset.post_process_multiclass(cav_content,
                             output_dict_ego, online_eval_only)
...
def fix_cavs_box_v2(pred_box_tensor, gt_box_tensor, pred_score, batch_data):
    """
    Fix the missing pred_box and gt_box for ego only (no other CAVs).
    Args:
        pred_box_tensor : tensor
            shape (N1, 8, 3), may or may not include ego agent prediction, but it should include
    ...
    """
    if pred_box_tensor is None or gt_box_tensor is None:
        return pred_box_tensor, gt_box_tensor, pred_score
    # Determine the number of agents (N)
    if 'record_len' in batch_data['ego']:
        N = batch_data['ego']['record_len']
    else:
        N = len(batch_data)

    # Create ego's bounding box
    extent = [2.45, 1.06, 0.75]
    ego_box = create_bbx(extent).reshape(1, 8, 3)  # [1, 8, 3]
    ego_box[..., 2] -= 1.85  # Adjust z-coordinate (hard coded)
    ego_box[..., 0] -= 1.3  # Adjust x-coordinate (hard coded)

    ego_box_tensor = torch.tensor(ego_box, device=pred_box_tensor.device)

    # Concatenate ego's box with existing pred and gt boxes
    pred_box_tensor_ = torch.cat((ego_box_tensor, pred_box_tensor), dim=0)
    gt_box_tensor_ = torch.cat((ego_box_tensor, gt_box_tensor), dim=0)

    # Update scores: assign score 1.0 for ego's box
    pred_score_ = torch.cat((torch.ones(1, device=pred_score.device), pred_score))

    # Assign scores for gt boxes: 1.0 for ego, 0.5 for others
    gt_score_ = torch.ones(gt_box_tensor_.shape[0], device=pred_box_tensor.device)
    gt_score_[1:] = 0.5
