...
def main():
    ...
    if opt.fusion_method == 'early':
        if opt.mode == 'cv':
            hypes['test_dir']= "dataset/V2XSET/test_cv"
        if opt.mode == 'inf':
            hypes['test_dir']= "dataset/V2XSET/test_inf"
        hypes1['test_dir']= "dataset/V2XSET/test"

    if 'heter' in hypes:
        hypes1['heter']['assignment_path'] = 'opencood/logs/heter_modality_assign/v2xset_4modality_all_2.json'
        if opt.mode == 'cv':
            hypes['heter']['assignment_path'] = 'opencood/logs/heter_modality_assign/v2xset_4modality_cv_2.json'
        elif opt.mode == 'inf':
            hypes['heter']['assignment_path'] = 'opencood/logs/heter_modality_assign/v2xset_4modality_inf_2.json'
    ...
    for i, (batch_data, batch_data_1, batch_data_2) in enumerate(zip(data_loader, data_loader1, data_loader2)):
        ...
        with torch.no_grad():
            ...
            gt, det, score= eval_utils_v2.RQ2a(pred_box_tensor,
                        pred_score,
                        gt_box_tensor,
                        pred_box_tensor_1,
                        pred_score_1,
            ...
            eval_utils_v2.caluclate_tp_fp_v2(det,
                                    score,
                                    gt,
                                    result_stat_2,
                                    0.5)
