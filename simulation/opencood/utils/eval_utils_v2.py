...
def caluclate_tp_fp(det_boxes, det_score, gt_boxes, result_stat, iou_thresh):
    ...
    if det_boxes is not None:
        ...
        for i in range(score_order_descend.shape[0]):
            ...
            if len(gt_polygon_list) == 0 or np.max(ious) < iou_thresh: # max_iou <= iou_thresh
                fp.append(1)
                tp.append(0)
                # 2. false detection
                if max_iou <= 0 :
...
#         gt_boxes = common_utils.torch_tensor_to_numpy(gt_boxes)
        ...
        for i in range(score_order_descend.shape[0]):
            ...
            gt_polygon_list.pop(gt_index)
        result_stat[iou_thresh]['score'] += det_score.tolist()
    result_stat[iou_thresh]['det'] = len(det_boxes)
    result_stat[iou_thresh]['fp'] += fp
    result_stat[iou_thresh]['tp'] += tp
    result_stat[iou_thresh]['gt'] += gt
...
#     for iou_thresh in [0.30, 0.50, 0.70]:
    for iou_thresh in [0.50]:
        ...
        fp_total_single = np.sum(result_stat[iou_thresh]['fp_single'])
        tp_total_single = np.sum(result_stat[iou_thresh]['tp_single'])
        gt_total_single = result_stat[iou_thresh]['gt_single']
        ...
        fn_total_single = gt_total_single - tp_total_single
        ...
        dump_dict.update({
            ...
            f'FP_SINGLE_{int(iou_thresh*100)}': int(fp_total_single),
            f'TP_SINGLE_{int(iou_thresh*100)}': int(tp_total_single),
            f'FN_SINGLE_{int(iou_thresh*100)}': int(fn_total_single),

            f'AP_{int(iou_thresh*100)}': float(locals()[f'ap_{int(iou_thresh*100)}']),
            
            f'GT_{int(iou_thresh*100)}': int(gt_total),
            f'GT_SINGLE_{int(iou_thresh*100)}': int(gt_total_single),
            ...
            f'CCME_{int(iou_thresh*100)}': int(s4_total),
            f'CCLE_{int(iou_thresh*100)}': int(s5_total),
...
def compare(det_boxes, det_score, gt_boxes, single_det, single_score, single_gt, result_stat, iou_thresh):
    ...
    if det_boxes is not None and single_det is not None and gt_boxes is not None and single_gt is not None:
        ...
        # 遍历每一个检测框，计算它与所有真值框的最大 IoU。如果最大 IoU 小于阈值 iou_thresh，则将其标记为假阳性（FP）；否则标记为真阳性（TP）。
        for i in range(score_order_descend.shape[0]):
            det_polygon = det_polygon_list[score_order_descend[i]]
            ious = common_utils.compute_iou(det_polygon, gt_polygon_list)
