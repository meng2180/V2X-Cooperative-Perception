...
def update_dict(d1,d2):
    ...
    :param d1: Default nested dictionary,
    :param d2: Updated dictionary
    :return d1:
...
def compute_iou(box, boxes):
    ...
    for b in boxes:
        ...
        safe_union_area = union_area + 1e-6

        iou_value = box.intersection(b).area / safe_union_area
        iou.append(iou_value)

    for idx, b in enumerate(boxes):
        union = box.union(b)
        if union.area <= 1e-6:
            print(f"Invalid box pair detected (index {idx}):")
            print(f"Box 1: {list(box.exterior.coords)}")
            print(f"Box 2: {list(b.exterior.coords)}")
    return np.array(iou, dtype=np.float32)
