...
class DynamicObjectCrossing(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        # Log trigger point and reference waypoint for debugging
        print(f"Trigger point: {config.trigger_points[0].location}")
        print(f"Reference waypoint: {waypoint.transform.location}, yaw: {waypoint.transform.rotation.yaw}")
        ...
        while True:  # We keep trying to spawn avoiding props
            ...
            try:
                if vehicle_waypoint:  # vehicle_waypoint:  False
                    ...
                    # Log spawn positions
                    print(f"Dynamic actor position: {self.transform.location}, yaw: {self.transform.rotation.yaw}")
                    print(f"Blocker vehicle position: {self.transform2.location}, yaw: {self.transform2.rotation.yaw}")
                else:
                    ...
                    # Log spawn positions
                    print(f"Dynamic actor position: {self.transform.location}, yaw: {self.transform.rotation.yaw}")
                    print(f"Blocker prop position: {self.transform2.location}, yaw: {self.transform2.rotation.yaw}")
                ...
                if self._spawn_attempted >= self._number_of_attempts:
                    print(f"Failed to spawn actors after {self._number_of_attempts} attempts. Error: {r}")
                    raise r
