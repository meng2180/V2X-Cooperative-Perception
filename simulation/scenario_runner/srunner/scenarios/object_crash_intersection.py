...
class VehicleTurningRight(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        # actor_traverse = DriveDistance(self.other_actors[0], dist_to_travel)
...
class VehicleTurningLeft(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        bycicle_start_dist = 13 + dist_to_travel
        dist_to_travel = 4 * lane_width + (1.10 * lane_width * self._num_lane_changes) # cs 4 * lane_width
...
class VehicleTurningRoute(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        # Move a certain distance to the front
                # allow the turning right scenario to be complicated   
        if self.sidewalk_flag:
            other_actor_loacation, _ = get_location_previous_from_wp(waypoint,6, stop_at_junction=False) # True
    ...
    def _create_behavior(self):
        ...
        if not self.sidewalk_flag:
            dist_to_travel = lane_width + (1.10 * lane_width * self._num_lane_changes)
            # dist_to_travel = 4 * lane_width + (1.10 * lane_width * self._num_lane_changes) # cs
        else:

            dist_to_travel = 4 * lane_width + self._pass_distance + (1.10 * lane_width * self._num_lane_changes) + self.accum_distance
            # dist_to_travel = 4 * lane_width + (1.10 * lane_width * self._num_lane_changes) + self.accum_distance
