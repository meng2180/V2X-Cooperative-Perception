...
def calculation_error(infer_result, Mis_error, timestamp):
	
	cp_det_boxes = common_utils.torch_tensor_to_numpy(infer_result['pred_box_tensor'])
	det_score = common_utils.torch_tensor_to_numpy(infer_result['pred_score'])
	gt_boxes = common_utils.torch_tensor_to_numpy(infer_result['gt_box_tensor'])
	ego_det_boxes = common_utils.torch_tensor_to_numpy(infer_result['ego_pred_box_tensor'])
	gt = gt_boxes.shape[0]
	...
	for i in range(gt):
		...
		if Detected_bounding_box(gt_polygon, ego_det_polygon_list, 0.5):
			if Detected_iou(gt_polygon, cp_det_polygon_list, iou_thresh = 0):
				Mis_error["global"]["LCME"] += 1
			elif Detected_iou(gt_polygon, cp_det_polygon_list, iou_thresh = 0.5, iou_min=0):
				Mis_error["global"]["LCLE"] += 1
		else:
			if Detected_iou(gt_polygon, cp_det_polygon_list, iou_thresh = 0):
				Mis_error["global"]["CCME"] += 1
			elif Detected_iou(gt_polygon, cp_det_polygon_list, iou_thresh = 0.5, iou_min=0):
				Mis_error["global"]["CCLE"] += 1
	...
	for i in range(len(cp_det_polygon_list)):
		...
		if  Detected_iou(cp_polygon, gt_polygon_list, iou_thresh = 0):
			if Detected_iou(cp_polygon, ego_det_polygon_list, iou_thresh = 0):
				Mis_error["global"]["LADE"] += 1
			else:
				Mis_error["global"]["CADE"] += 1
	Mis_error["global"]["GT"] += gt

	Mis_error["last"].append(copy.deepcopy(Mis_error["global"]))
...
def calculation_ap(result_stat_all, iou_thresh):
	overall = {0.5: {'tp': [], 'fp': [], 'gt': 0, 'score': []}}


	if iou_thresh in result_stat_all and result_stat_all[iou_thresh]['tp']:
		overall[0.5]['tp'].extend(result_stat_all[iou_thresh]['tp'])
		overall[0.5]['fp'].extend(result_stat_all[iou_thresh]['fp'])
		overall[0.5]['gt'] += result_stat_all[iou_thresh]['gt']
		overall[0.5]['score'].extend(result_stat_all[iou_thresh]['score'])
...
class DisplayInterface(object):
	...
	def run_interface(self, input_data):
		...
		surface = np.zeros((600, 1100, 3),np.uint8)
		surface[:, :800] = rgb
...
class PnP_infer():
	def __init__(self, config=None, ego_vehicles_num=1, perception_model=None, planning_model=None, perception_dataloader=None, device=None) -> None:
		...
		self.det_range = [36, 12, 14, 14, 0.25]
		self.config['perception']['perception_hypes']['postprocess']['gt_range'] = [-36, -20, -22, 36, 20, 14]
		...
		# class_list = [0,1,3]
		# for c in class_list:
		# 	self.result_stat[c] = {0.5: {'tp': [], 'fp': [], 'gt': 0, 'score': []}}
		self.result_stat = {0.5: {'tp': [], 'fp': [], 'gt': 0, 'score': []}}
		self.t = [False ,0, 0, 0, 0]
		self.Mis_error = {
			"global": {
				"LCME": 0, "LCLE": 0, "LADE": 0, "CCME": 0, "CCLE": 0, "CADE": 0, "GT": 0
			},
			"last": deque(), 
			"path": None
		}
	...
	def get_action_from_list_inter(self, car_data_raw, rsu_data_raw, step, timestamp, perception_method='cp'):
		'''
		...
		pred_box_tensor, pred_score, gt_box_tensor = \
			self.perception_dataloader.post_process_multiclass_gt(batch_data_perception,
								output_dict, online_eval_only=False) 
		
		ego_pred_box_tensor_list = pred_box_tensor
		...
		if not os.path.exists(folder_path):
			...
			# self.Mis_error["path"] = os.path.join(self.save_path,"Error.json")
		...
		gt_feature = None
		gt_feature_dir = 128
		if self.perception_model.__class__.__name__ == 'PointPillarMulticlass':
			gt_feature_dir = 256
		gt_box_tensors = torch.cat(gt_box_tensor, dim=0) if gt_box_tensor else None  # (N, 8, 3)
		gt_feature = self.gt_to_feature(gt_box_tensors, self.ego_vehicles_num, 5, 192, 96, gt_feature_dir)
		
		planning_input = self.generate_planning_input(gt_feature) # planning_input['occupancy'] [1, 5, 6, 192, 96] planning_input['target'] [1,2]
	...
	def generate_planning_input(self, gt_feature=None):
		...
		##################  end2end feature #####################
		feature_dim = self.perception_memory_bank[0]['feature'].shape[1]
		feature_to_warp = torch.zeros(self.ego_vehicles_num, 5, feature_dim, 192, 96).cuda().float()

		if gt_feature is not None:
			alpha = 0.5
			feature_to_warp = alpha * feature_to_warp + (1 - alpha) * gt_feature
		feature_warped_list = []
		##########################
	...
	def gt_to_feature(self, gt_box_tensor, batch_size, time_steps, height, width, feature_dir = 128):
		"""

		Args:
			gt_box_tensor: torch.Tensor of shape (N, 8, 3), groundtruth bbx
			batch_size: int, number of ego vehicles (B)
			time_steps: int, number of time steps (T)
			height: int, feature height (H)
			width: int, feature width (W)
		
		Returns:
			torch.Tensor: feature tensor of shape [B, T, 128, H, W]
		"""
		# v2x-vit:256
		B, T, C2, H, W = batch_size, time_steps, feature_dir, height, width
		
		if gt_box_tensor is None or len(gt_box_tensor) == 0:
			return torch.zeros(B, T, C2, H, W).to(self.device)

		occ_map = torch.zeros(B, T, H, W).to(self.device)
		resolution = self.det_range[-1]

		for b in range(B):
			if gt_box_tensor is not None:
				gt_boxes = gt_box_tensor.cpu().numpy()  # [N, 8, 3]
				for box in gt_boxes:
					corners = box[:, :2]
					grid_x = ((corners[:, 0] - self.det_range[2]) / resolution).astype(int)
					grid_y = ((corners[:, 1] - self.det_range[1]) / resolution).astype(int)
					
					x_min, x_max = np.clip(grid_x.min(), 0, W-1), np.clip(grid_x.max(), 0, W-1)
					y_min, y_max = np.clip(grid_y.min(), 0, H-1), np.clip(grid_y.max(), 0, H-1)
					
					occ_map[b, -1, y_min:y_max+1, x_min:x_max+1] = 1.0
				
				occ_map[b, :-1] = occ_map[b, -1].unsqueeze(0).repeat(T-1, 1, 1)

		occ_map = occ_map.view(-1, 1, H, W)  # [B*T, 1, H, W]
		with torch.no_grad():
			feature_conv = nn.Conv2d(1, feature_dir, kernel_size=3, padding=1).to(self.device)
			feature = feature_conv(occ_map)  # [B*T, 256, H, W]
		feature = feature.view(B, T, C2, H, W)
