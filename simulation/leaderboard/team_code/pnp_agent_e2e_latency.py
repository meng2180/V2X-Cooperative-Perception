...
from simulation.leaderboard.team_code.pnp_infer_action_e2e_eval import PnP_infer
...
class PnP_Agent(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, ego_vehicles_num):
        ...
        # load agent config
        # path_to_conf_file: simulation/leaderboard/team_code/agent_config/*****.yaml
        ...
        match = re.search(r"_r(\d+)", routes_path)
        route_id = int(match.group(1))# 7
        comm_latency = (1664524  * route_id + 1013904223) % (2**32)
        # comm_latency = (200000002 * route_id + 1013904223) % (2**32)
        comm_latency = (comm_latency % 3 + 3) * 4
        self.config['simulation'].update({"comm_latency": comm_latency})
    ...
    def _init(self):
        ...
        """
        # team_code.planner_pnp = RoutePlanner
        ...
        # team_code.utils.carla_birdeye_view = BirdViewProducer
