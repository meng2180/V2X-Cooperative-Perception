...
from leaderboard.scenarios.scenarioatomics.atomic_criteria_eval import (CustomCollisionTest,
                                                                     CustomInRouteTest,
                                                                     CustomRouteCompletionTest,
                                                                     CustomOutsideRouteLanesTest,
                                                                     CustomActorSpeedAboveThresholdTest)
...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True, ego_vehicles_num=1,log_dir=None, scenario_parameter=None, mis_error=None, trigger_distance=10):
        """
    ...
    def _scenario_sampling(self, potential_scenarios_definitions, random_seed=0):
        ...
        for trigger in potential_scenarios_definitions.keys():
            ...
            print('load {} at (x={}, y={}, yaw={})'.format(scenario_choice['name'], scenario_choice['trigger_position']['x'], scenario_choice['trigger_position']['y'],
                                                           scenario_choice['trigger_position']['yaw']))
    ...
    def _build_scenario_parameter_instances(self, world:carla.libcarla.World, ego_vehicles: List, scenario_definitions: List,
        ...
        for j in range(len(scenario_definitions)):
            ...
            for scenario_number, definition in enumerate(scenario_definition):
                ...
                print(f"load scenario: {scenario_class_name}")
                ...
                # print("###################{}".format(scenario_instance.other_actors[0].get_transform().location))
                # print("###################{}".format(scenario_instance.other_actors[0].get_transform().rotation))
                # Do a tick every once in a while to avoid spawning everything at the same time
                if scenario_number % scenarios_per_tick == 0:
                    if CarlaDataProvider.is_sync_mode():
                        world.tick()
                ...
                #     print("Skipping scenario '{}' due to setup error: {}".format(definition['name'], e))
                #     continue  
                loc1 = scenario_instance.other_actors[0].get_transform().location
                if scenario_instance_vec:
                    loc2 = scenario_instance_vec[-1].other_actors[-1].get_transform().location
                    xy_distance = ((loc1.x - loc2.x) ** 2 + (loc1.y - loc2.y) ** 2) ** 0.5
                    if xy_distance < 1:
                        continue
    ...
    def _initialize_actors(self, config):
        ...
        import os
        import json
        log_file = f"./route_record_point/{config.name}.json"
        ...
        actors_len = {
            "vehicle.*": len(new_actors_vehicle), 
            'walker.pedestrian.*': len(new_actors_pedestrian), 
            'vehicle.diamondback.century': len(new_actors_bicycle)
        }
        ...
        if new_actors:
            ...
            for actor in new_actors:
                ...
                model = 'vehicle.*' if actor.type_id.startswith('vehicle.') and actor.type_id != 'vehicle.diamondback.century' else actor.type_id
                if model not in actors_by_type:
                    model = 'vehicle.diamondback.century' if actor.type_id == 'vehicle.diamondback.century' else 'walker.pedestrian.*'
                
                transform = actor.get_transform()
                location = transform.location
                rotation = transform.rotation
                actors_by_type[model].append({
                    'actor_id': actor.id,
                    'type': model,
                    'rolename': actor.attributes.get('role_name', 'background'),
                    'location': {'x': location.x, 'y': location.y, 'z': location.z},
                    'rotation': {'pitch': rotation.pitch, 'yaw': rotation.yaw, 'roll': rotation.roll}
                })

            
            for model, entries in actors_by_type.items():
                with open(log_file, 'a') as f:
                    json.dump({'actors': entries, 'type': model, 'len': actors_len[model]}, f, indent=2)
                    f.write('\n')  
    ...
    def _create_test_criteria(self):
        ...
        for ego_vehicle_id in range(len(self.list_scenarios)):
            ...
            collision_criterion = CustomCollisionTest(self.ego_vehicles[ego_vehicle_id], terminate_on_failure=True, mis_error=self.mis_error)
            route_criterion = CustomInRouteTest(self.ego_vehicles[ego_vehicle_id],
                                        route=route,
                                        offroad_max=30,
                                        terminate_on_failure=True,
                                        mis_error=self.mis_error)
                                        
            completion_criterion = CustomRouteCompletionTest(self.ego_vehicles[ego_vehicle_id], route=route, mis_error=self.mis_error)

            outsidelane_criterion = CustomOutsideRouteLanesTest(self.ego_vehicles[ego_vehicle_id], route=route, mis_error=self.mis_error)
            ...
            blocked_criterion = CustomActorSpeedAboveThresholdTest(self.ego_vehicles[ego_vehicle_id],
                                                            speed_threshold=1, # 0.5,
                                                            below_threshold_max_time=15.0,
                                                            terminate_on_failure=True,
